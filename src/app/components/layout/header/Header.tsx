...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-foreground/20 backdrop-blur-md">
      <div className="container max-w-6xl mx-auto flex items-center justify-between lg:px-4 px-2 py-3">
        <a href="#" className="font-display text-xl font-bold text-cream flex-auto">
          <PasikaLogo size="sm" />
        </a>
        ...
        <div className="hidden lg:flex items-center gap-8 mr-10">
          ...
          <button
            onClick={toggleCart}
            className="relative text-cream/80 hover:text-cream transition-colors"
        ...
        <div
          ...
          onClick={toggleMenu}>
          <span
            className={`block h-0.5 bg-white transition-all duration-300 ${isOpen ? "rotate-45 translate-y-1" : ""
              }`}></span>
          <span
            className={`block h-0.5 bg-white transition-all duration-300 ${isOpen ? "opacity-0" : ""
              }`}></span>
          <span
            className={`block h-0.5 bg-white transition-all duration-300 ${isOpen ? "-rotate-45 -translate-y-[12px]" : ""
              }`}></span>
        </div>
      </div>
      ...
      {isOpen && (
        <div className="lg:hidden bg-foreground/90 backdrop-blur-md border-t border-cream/10 px-4 pb-4">
          ...
          {/* <a
            href="#contacts"
            onClick={() => setIsOpen(false)}
            className="block mt-2 bg-golden-gradient text-primary-foreground text-sm font-semibold px-5 py-2 rounded-lg text-center hover:opacity-90 transition-opacity"
          >
            Замовити
          </a> */}
        </div>
